...
alter table if exists carts_products
    add constraint FKt3mepi19unnkcmw4683q5wr39 foreign key (product_id) references products;
alter table if exists carts_products
    add constraint FK3nvguygrfbn661omvvu2uafu5 foreign key (cart_id) references carts;
alter table if exists categories
    add constraint FKbvxj7yyy7wyrp1gwoksfrd5ji foreign key (order_id) references orders;
alter table if exists categories
    add constraint FKb4c3eeiugg1svyrcheyd4yo63 foreign key (product_id) references products;
alter table if exists orders
    add constraint FK32ql8ubntj5uh44ph9659tiih foreign key (user_id) references users;
alter table if exists orders_details
    add constraint FKipixxqtl6ooh776qk4wit5kui foreign key (details_id) references categories;
alter table if exists orders_details

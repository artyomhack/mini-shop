...
alter table if exists carts add constraint FKb5o626f86h46m4s7ms6ginnop foreign key (user_id) references userEntities;
...
alter table if exists carts_products add constraint FKt3mepi19unnkcmw4683q5wr39 foreign key (product_id) references productEntities;
alter table if exists categories add constraint FKbvxj7yyy7wyrp1gwoksfrd5ji foreign key (order_id) references orders;
alter table if exists categories add constraint FKb4c3eeiugg1svyrcheyd4yo63 foreign key (product_id) references productEntities;
alter table if exists orders add constraint FK32ql8ubntj5uh44ph9659tiih foreign key (user_id) references userEntities;
...
alter table if exists products_categories add constraint FKtj1vdea8qwerbjqie4xldl1el foreign key (product_id) references productEntities;
alter table if exists userEntities add constraint FKdv26y3bb4vdmsr89c9ppnx85w foreign key (cart_id) references carts;